...
public class DataService {
    ...
    public void writeList(CarList carList, String filename) {
        ...
        try { om.writerWithDefaultPrettyPrinter().writeValue(file, carList); } catch(IOException e) { e.printStackTrace(); }
    }
    public CarList readList(String filename) {
        File path = context.getExternalFilesDir(null);
        File file = new File(path, filename);
        CarList returnList = new CarList();
        try { returnList = om.readValue(file, CarList.class); } catch(IOException e) { e.printStackTrace(); }
        return returnList;
    }
}
